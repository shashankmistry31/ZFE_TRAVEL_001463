...
CLASS LHC_TRAVEL IMPLEMENTATION.
  ...
  READ ENTITIES OF ZFE_R_TravelTP_001463 IN LOCAL MODE
    ENTITY Travel
  ...
  SELECT MAX( TRAVEL_ID ) FROM ZFE_ATRAV_001463 INTO @DATA(max_object_id).
  "Add support for draft if used in modify
  "SELECT SINGLE FROM FROM ZFE_DTRAV_001463 FIELDS MAX( TravelID ) INTO @DATA(max_orderid_draft). "draft table
  "if max_orderid_draft > max_object_id
  " max_object_id = max_orderid_draft.
  "ENDIF.
  MODIFY ENTITIES OF ZFE_R_TravelTP_001463 IN LOCAL MODE
    ENTITY Travel
